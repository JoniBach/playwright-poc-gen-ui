...
async function handleIndexCommand(): Promise<void> {
  const spinner = createSpinner(MESSAGES.index.generating).start();

  try {
    await genIndex();
    spinner.succeed(chalk.green(MESSAGES.index.success));
    process.exit(0);
  } catch (error) {
    handleCommandError(spinner, 'journey index', error);
  }
}
...
async function handleAllCommand(): Promise<void> {
  ...
  const tasks = new Listr([
    {
      title: chalk.cyan('📋 Generating journey index'),
      task: async () => await genIndex()
    },
    {
